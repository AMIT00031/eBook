<div class="m-b-10">
    ...
    <div class="pull-right">
       <a href="<?php echo base_url('admin/product/addedit'); ?>" class="btn btn-primary">Add Product</a> 
...
<tbody>
    ...
    if (!empty($lists)) {
        foreach ($lists as $list) { 
				  $imgsrc = 'http://dnddemo.com/ebooks/api/v1/upload/books/'.$list->thubm_image;
				  $default = base_url().'assets/front/images/image-not-found.jpg';
            ?>
            <tr role="row" class="even">
				<td><?php echo!empty($list->book_title) ? $list->book_title : '' ?></td>
			    <td><?php echo!empty($list->category_id) ? $list->category_id : 'N/A' ?></td>
               <?php if(!$list->thubm_image){ ?>
				...
				<td><?php echo!empty($list->book_slug) ? $list->book_slug : 'N/A' ?></td>
				...
				<td>
                    <a class="btn btn-primary" href="<?php echo base_url('admin/product/addedit/' . $list->id) ?>">Edit</a>       
...
function changeStatus(id){
	...
	if (r == true) {
        if (ckbox.is(':checked')) {
			$.ajax({
				type: 'post',
				url : base_url+'admin/product/changeStatus/'+id+'/1',
			...
			$.ajax({
				type: 'post',
				url : base_url+'admin/product/changeStatus/'+id+'/0',
				success:function(data){ 
					if(data==0){ 
						alert('Status changed successfully');
					}
